using BasicWeatherCacheApp.Models;
...
public class WeatherForecastController : ControllerBase
{
    ...
    public async Task<ForecastResult> Get([FromQuery] double latitude, [FromQuery] double longitude)
    {
        ...
        {
            json = await GetForecastAsync(latitude, longitude);
            var setTask = _redis.StringSetAsync(keyName, json);
            var expireTask = _redis.KeyExpireAsync(keyName, TimeSpan.FromHours(1));
            await Task.WhenAll(setTask, expireTask);
        }

        var forecast = JsonSerializer.Deserialize<IEnumerable<Models.WeatherForecast>>(json);
        watch.Stop();
    ...
    private async Task<string> GetForecastAsync(double latitude, double longitude)
    {
